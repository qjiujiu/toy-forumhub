...
class UserService:
    ...
    def _verify_admin(self, admin_uid: str) -> None:
        ...
        Raises:
            AdminPermissionDenied: 非管理员操作
        """
        user = self._user_repo.get_user_by_uid(admin_uid)
        if not user:
    ...
    def create_user(self, user_data: UserDto, to_dict: bool = True) -> Union[Dict, UserOut]:
        """
        创建用户：
        1. 对明文密码做 Argon2 哈希
    ...
    def hard_delete_user(self, admin_uid: str, user_uid: str) -> bool:
        ...
        Args:
            admin_uid: 管理员uid（用于鉴权）
            user_uid: 用户ID（被操作的用户）
        ...
        ok = self._user_repo.hard_delete_user(user_uid)
        if ok:
    ...
    def ban_user_by_uid(self, admin_uid: str, user_uid: str, to_dict: bool = True) -> Optional[Union[Dict, UserOut]]:
        ...
        updated = self._user_repo.ban_user_by_uid(user_uid)
        if not updated:
            return None
    ...
    def frozen_user_by_uid(self, admin_uid: str, user_uid: str, to_dict: bool = True) -> Optional[Union[Dict, UserOut]]:
        ...
        updated = self._user_repo.frozen_user_by_uid(user_uid)
        if not updated:
            return None
    ...
    def user_to_admin(self, admin_uid: str, user_uid: str, to_dict: bool = True) -> Optional[Union[Dict, UserOut]]:
        ...
        updated = self._user_repo.user_to_admin(user_uid)
        if not updated:
            return None
    ...
    def user_to_moderator(self, admin_uid: str, user_uid: str, to_dict: bool = True) -> Optional[Union[Dict, UserOut]]:
        ...
        updated = self._user_repo.user_to_moderator(user_uid)
        if not updated:
            return None
